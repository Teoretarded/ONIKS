...
export const ANATOMY = {
  ...
  tel: {
    ...
    parts: [
      ...
      E(null, ['cabIn'], 'Cab interior · seats ×3 · controls', '', [0, 2.6, 2.2], .16, 'part', { anchor: [-.74, 2.3, 4.6] }),
  ...
  radar: {
    ...
    parts: [
      ...
      E('09', ['drive'], 'Transmission · transfer case · cardan shafts', '', [0, .2, 0], .4, 'part', { anchor: [.37, 1.1, 3.5], side: 'L' }),
  ...
  pantsir: {
    ...
    parts: [
      ...
      E('14', ['drive'], 'Transfer case · cardan shafts · hub reductions', '', [0, .1, 0], .48, 'part', { anchor: [.3, .7, .2], side: 'L' }),
      E('11', ['cab'], 'Cab · KamAZ-6560 · crew 3', '', [0, .95, 2.4], .38, 'shell', { side: 'R' }),
      E(null, ['cabIn'], 'Cab interior · seats ×3 · controls', '', [0, 1.7, 2.9], .4, 'part', { anchor: [-.64, 2.0, 3.8] }),
  ...
  drone: {
    ...
    note: 'Small reconnaissance aircraft: tractor propeller, high wing, an EO/IR gimbal under the fuselage; thrown off its rail from a shoe under the belly, recovered by parachute.',
    view: { yaw: -.9, pitch: .4 },
    parts: [
      ...
      E('02', ['prop'], 'Propeller · 2 blades · tractor', '', [0, 0, .4], .1, 'part', { side: 'R' }),
      ...
      E('06', ['engine'], 'Engine · single cylinder · 4-stroke', '', [0, .02, .3], .38, 'part', { side: 'R' }),
      E('07', ['fuel'], 'Fuel tank', '', [0, -.1, 0], .42, 'part', { side: 'L' }),
      E('08', ['bays'], 'Avionics · datalink · battery · camera · parachute', '', [0, .16, -.1], .4, 'part', { side: 'L' }),
      E('09', ['fuselage'], 'Fuselage · Orlan-10', '1.8 m', [0, 0, 0], .5, 'shell', { side: 'L' }),
    ],
    xray: [],
  },
  catapult: {
  ...
  transloader: {
    model: 'transloader_cut', frame: 'chassis', title: 'K342P transloader', size: '14.0 × 3.1 m',
    ...
    parts: [
      ...
      E(null, ['cabIn'], 'Cab interior · seats ×3 · controls', '', [0, 2.6, 2.2], .16, 'part', { anchor: [-.74, 2.3, 4.6] }),
      E('08', ['engine'], 'YaMZ-846 · V12 · radiators ×2', '500 hp', [0, 2.3, .4], .3, 'part', { side: 'R' }),
      E('09', ['drive'], 'Transmission · transfer case · cardan shafts', '', [0, .2, 0], .45, 'part', { anchor: [.37, 1.1, 3.5], side: 'L' }),
  ...
  destroyer: {
    ...
    parts: [
      ...
      E(null, ['hangarIn'], 'Hangars · interior · RSD tracks', '', [0, 9, -12], .38, 'part'),
      E('13', ['helo'], 'MH-60R Seahawk · blades and pylon folded', '', [0, 3.6, -21], .4, 'part', { side: 'L' }),
  ...
  carrier: {
    ...
    xray: [
      { id: 'HB', label: 'F/A-18E Super Hornet ×8 · in hangar bays 1–2', model: 'fighter', st: { gear: 1, fold: 1 }, parent: 'hangar',
        inst: () => [[-8.2, 64], [8.2, 52], [-8.2, 38], [8.2, 26], [-8.2, -6], [8.2, -20], [-8.2, -34], [8.2, -48]].map(([x, z]) => T3([x, 8.3 + 1.51, z])) },
      { id: 'AW', label: 'E-2D Hawkeye ×2 · in hangar bay 3', model: 'aew', st: { fold: 1, dome: 0, prop: 0, gear: 1 }, parent: 'hangar',
        inst: () => [[-7.5, -86], [7.5, -106]].map(([x, z]) => T3([x, 8.3 + 2.1, z])) },
    ],
  },
  helo: {
    model: 'helo_cut', title: 'MH-60R Seahawk', size: '19.8 m · rotor Ø 16.4 m · 12.5 m folded',
    note: 'Navy multi-mission helicopter: two T700 turboshafts, four-blade main rotor, canted tail rotor, radome and FLIR, dipping sonar; blades and tail pylon fold for the hangar.',
    view: { yaw: -1.0, pitch: .22 },
    parts: [
      E('01', ['rotor'], 'Main rotor · 4 blades · elastomeric head', 'Ø 16.36 m', [0, 3.2, 0], 0, 'part', { side: 'R' }),
      E('02', ['tailrotor'], 'Tail rotor · canted 20°', 'Ø 3.35 m', [1.3, .6, -3.0], .1, 'part', { side: 'L' }),
      E('03', ['tail'], 'Tail pylon · folds to port · stabilators', '', [0, .4, -2.6], .16, 'part', { side: 'L' }),
      E(null, ['tailDrive'], 'Intermediate · tail gearboxes', '', [0, 1.2, -3.4], .2, 'part'),
      E('04', ['gearbox'], 'Main gearbox · tail drive shaft', '', [0, 2.4, 0], .12, 'part', { side: 'R' }),
      E('05', ['engines'], 'T700-GE-401C ×2', '1 900 shp', [0, 3.0, 1.6], .08, 'part', { side: 'R' }),
      E('06', ['sensors'], 'MTS FLIR · AN/APS-153 · AQS-22 dome', '', [0, -1.0, 1.4], .25, 'part', { side: 'R' }),
      E('07', ['pylonsR'], 'Pylons · Mk 54 ×2 · M299 · AGM-114 ×4', '', [1.4, -.3, 0], .35, 'part', { side: 'R' }),
      E(null, ['pylonsL', 'pylonsC'], 'Pylons · L', '', [-1.4, -.3, 0], .35, 'part'),
      E('08', ['gearR'], 'Landing gear · mains · twin tail wheels', '', [1.1, -.9, 0], .3, 'part', { side: 'R' }),
      E(null, ['gearL'], 'Main gear · L', '', [-1.1, -.9, 0], .3, 'part'),
      E(null, ['gearT'], 'Tail wheels · RAST probe', '', [0, -.9, -.8], .3, 'part'),
      E('09', ['cabin'], 'Cockpit · cabin · SO station · ALFS reel', '', [0, -.4, 1.0], .42, 'part', { side: 'L' }),
      E('10', ['fuel'], 'Fuel cells ×2', '', [0, .6, -2.2], .44, 'part', { side: 'L' }),
      E('11', ['fuselage'], 'Fuselage · MH-60R', '19.76 m', [0, 0, 0], .5, 'shell', { side: 'L' }),
    ],
    xray: [],
  },
  fighter: {
    model: 'fighter_cut', title: 'F/A-18E Super Hornet', size: '18.3 × 13.6 m',
    note: 'Carrier strike fighter: two F414-GE-400 behind S-ducts and radar blockers, twin canted tails, outer wings that fold for the deck.',
    st: { gear: 1, probe: 1 }, view: { yaw: -.9, pitch: .45 },
    parts: [
      E('01', ['canopy'], 'Canopy · frame · HUD', '', [0, 2.0, .8], 0, 'part', { side: 'R' }),
      E('02', ['cockpit'], 'Cockpit · SJU-17 NACES seat', '', [0, 1.4, .8], .12, 'part', { side: 'R' }),
      E('03', ['radar'], 'AN/APG-79 AESA radar', '', [0, .6, 2.6], .08, 'part', { side: 'R' }),
      E(null, ['gun'], 'M61A2 · 20 mm · drum', '', [0, 1.1, 1.8], .1, 'part'),
      E(null, ['probe'], 'Refuelling probe', '', [.8, .6, 1.4], .05, 'part'),
      E('04', ['wingsR'], 'Wing · 13.62 m span · folds to 9.32 m', '13.62 m', [3.6, .3, 0], .2, 'part', { side: 'R' }),
      E(null, ['wingsL', 'wingsC'], 'Wing · L', '', [-3.6, .3, 0], .2, 'part'),
      E('05', ['tailsR'], 'Vertical tails ×2 · 20° cant', '', [1.4, 2.6, -.8], .15, 'part', { side: 'R' }),
      E(null, ['tailsL'], 'Tail · L', '', [-1.4, 2.6, -.8], .15, 'part'),
      E('06', ['stabsR'], 'Stabilators', '', [2.6, -.2, -2.4], .25, 'part', { side: 'R' }),
      E(null, ['stabsL'], 'Stabilator · L', '', [-2.6, -.2, -2.4], .25, 'part'),
      E('07', ['lexR'], 'Leading-edge extensions', '', [.8, .5, .8], .1, 'part', { side: 'L' }),
      E(null, ['lexL'], 'LEX · L', '', [-.8, .5, .8], .1, 'part'),
      E('08', ['intakesR'], 'Caret intakes · S-ducts · radar blockers', '', [1.6, -1.4, .8], .3, 'part', { side: 'L' }),
      E(null, ['intakesL', 'intakesC'], 'Intake · L', '', [-1.6, -1.4, .8], .3, 'part'),
      E('09', ['engines'], 'F414-GE-400 ×2', '22 000 lbf', [0, 1.5, -1.6], .32, 'part', { side: 'L' }),
      E('10', ['nozzlesR'], 'F414-GE-400 nozzles', '', [.7, 0, -3.2], .35, 'part', { side: 'L' }),
      E(null, ['nozzlesL', 'nozzlesC'], 'Nozzle · L', '', [-.7, 0, -3.2], .35, 'part'),
      E('11', ['fuel'], 'Fuel · fuselage cells · wing tanks', '', [0, 2.6, .3], .36, 'part', { side: 'R' }),
      E('12', ['storesR'], 'AIM-9X · AIM-120 · SLAM-ER · 480 gal tank', '', [3.2, -2.8, 0], .38, 'part', { side: 'R' }),
      E(null, ['storesL', 'storesC'], 'Stores · L · ATFLIR', '', [-3.2, -2.8, 0], .38, 'part'),
      E('13', ['pylonsR'], 'Pylons ×6 · nacelle stations ×2', '', [2.4, -2.2, 0], .4, 'part', { side: 'L' }),
      E(null, ['pylonsL'], 'Pylons · L', '', [-2.4, -2.2, 0], .4, 'part'),
      E('14', ['gearN'], 'Landing gear · twin nose wheels · launch bar', '', [0, -1.6, 1.4], .3, 'part', { side: 'L' }),
      E(null, ['gearL'], 'Main gear · L', '', [-1.2, -1.6, 0], .3, 'part'),
      E(null, ['gearR'], 'Main gear · R', '', [1.2, -1.6, 0], .3, 'part'),
      E(null, ['hook'], 'Arresting hook', '', [0, -1.2, -1.6], .34, 'part'),
      E('15', ['fuselage'], 'Fuselage · F/A-18E', '18.31 m', [0, 0, 0], .5, 'shell', { side: 'L' }),
    ],
    xray: [],
  },
  aew: {
    ...
    st: { dome: .5, prop: .3, fold: 0, gear: 1 }, view: { yaw: -.95, pitch: .36 },
    parts: [
      ...
      E(null, ['domeFrame'], 'Rotodome structure · ribs · rotary coupler', '', [0, 2.8, 0], .05, 'part'),
      E('13', ['gearN'], 'Landing gear · twin nose wheels · launch bar', '', [0, -1.6, 1.6], .34, 'part', { side: 'R' }),
      E(null, ['gearL'], 'Main gear · L', '', [-.8, -1.6, 0], .34, 'part'),
      E(null, ['gearR'], 'Main gear · R', '', [.8, -1.6, 0], .34, 'part'),
  ...
  bal: {
    ...
    parts: [
      ...
      E(null, ['cabIn'], 'Cab interior · seats ×3 · controls', '', [0, 2.6, 2.2], .14, 'part', { anchor: [-.74, 2.3, 4.6] }),
      E(null, ['bay'], 'Power pack cover', '', [0, 3.6, .4], .05, 'shell'),
      E('07', ['engine'], 'YaMZ-846 · V12 diesel', '500 hp', [0, 2.0, .2], .3, 'part', { side: 'R' }),
      E('11', ['drive'], 'Transmission · transfer case · cardan shafts', '', [0, .2, 0], .4, 'part', { anchor: [.37, 1.1, 3.5], side: 'L' }),
