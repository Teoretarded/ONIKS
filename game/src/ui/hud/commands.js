...
export function createCommands(game, hud, parent) {
  ...
  const items = new Map([...el.querySelectorAll('.it')].map(n => [n.dataset.c, { el: n, lab: n.querySelector('.lab'), x: n.querySelector('.x'), cls: '', labT: '', xT: '' }]));
  ...
  function run(id) {
    ...
    if (o && o.hotkey) o.hotkey(id === 'drone' ? 'drone' : id);
    hud.click();
    last = -1;
  }
  ...
  function evaluate() {
    ...
    const held = any && us.every(u => u.hold);
    S.hold = { ok: any, on: held, x: held ? 'on' : '', hint: held ? 'Release · weapons back to orders' : 'Hold position · offensive weapons pick their own targets' };
    const dep = us.filter(u => u.type === 'tel' || u.type === 'radar');
    if (dep.length) {
      const up = dep.every(u => u.type === 'tel' ? (u.dep > 0 || u.depT > 0 || u.elevT > 0) : (u.mast > 0 || u.mastT > 0));
      const blocked = dep.every(u => u.off.deploy);
      S.deploy = { ok: !blocked, lab: up ? 'Stow' : 'Deploy', x: blocked ? 'out' : '',
        hint: up ? 'Bring it down to drive' : dep[0].type === 'tel' ? 'Jacks 10 s · erect 15 s · cannot move while up' : 'Mast up 15 s · needed to radiate' };
    } else S.deploy = { ok: false, lab: 'Deploy', hint: 'TEL and Monolith-B only' };
    const rl = us.filter(u => u.type === 'tel' || u.type === 'transloader' || u.def.domain === 'sea' || u.type === 'pantsir');
    if (rl.length) {
      ...
      S.reload = { ok: need && !off, x: off ? 'out' : need ? '' : 'full',
        hint: rl[0].type === 'tel' ? 'A transloader alongside (45 s a round) or a depot (90 s)' : rl[0].type === 'transloader' ? 'Refill the TLCs at a depot' : rl[0].def.domain === 'sea' ? 'Sail to the replenishment point' : 'Refill at a depot' };
    } else S.reload = { ok: false, hint: 'TEL, transloader, Pantsir or ship' };
    ...
    S.drone = cs.length ? { ok: drones > 0, on: mode === 'drone', x: String(drones), hint: drones ? 'Orlan-10 off the rail · click the patrol point' : 'No drones on the rails' }
      : { ok: false, hint: 'Needs an Orlan-10 catapult' };
  ...
  return {
    ...
    update() {
      ...
      if (hover && state[hover]) h = `<b>${ROWS.find(r => r.id === hover).k}</b>${esc(state[hover].hint)}`;
        ...
        if (us.length) {
          const parts = [us.some(u => u.def.speed > 0) ? 'Move' : ''];
          if (us.some(u => Object.values(u.def.weapons).some(w => !w.gun && !(w.auto && !w.salvo)))) parts.push('on a track: attack');
          if (us.some(u => u.type === 'transloader')) parts.push('on a TEL: reload');
          if (us.some(u => u.def.domain === 'air' && u.type !== 'drone')) parts.push('on the carrier: land');
