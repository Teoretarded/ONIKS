...
export class Overlay {
  constructor(canvas) {
    ...
    this.ui = 1;               // UI scale (uiScaleOf the view, or uiScale when pinned)
    this.uiScale = null;
    this._w = new Map();
  ...
  tagSize(id, label, value, o) {
    const k = o && o.raw ? 1 : this.ui, px = ((o && o.size) || 11.5) * k, sp = px * .05, U = s => (s || '').toString().toUpperCase();
    this._font(px, 500);
    const sId = U(id), sL = U(label), sV = U(value), pX = px * .61, pL = px * .7;
    return [(sId ? this._meas(sId, px, sp) + pX * 2 : 0) + (sL ? this._meas(sL, px, sp) + pL * 2 : 0) + (sV ? this._meas(sV, px, sp) + pL * 2 : 0), Math.round(px + 9 * k)];
  }
  ...
  fitBox(x, y, w, h, avoid, margin) {
    const m = margin === undefined ? this.margin * this.ui : margin, W = this.W, H = this.H;
    ...
    for (let pass = 0; pass < 4; pass++) {
      ...
      const g = 10 * this.ui;
      for (const c of [[hit[0] - w - g, y], [hit[2] + g, y], [x, hit[1] - h - g], [x, hit[3] + g]]) {
  ...
  tag(x, y, id, label, value, o) {
    ...
    const pX = px * .61, pL = px * .7, hB = Math.round(px + 9 * k);
