/* Compiled hairline drawing for the HD models (local copy for OD "Salvo"). Each part's primitives are
   flattened once into typed arrays of segments with their facing data; a frame costs one local->camera
   transform per part and a facing test per segment. Alphas reproduce GEO.drawPrim: hex edges (front if
   either face faces the eye, else back), ribs, lathe rings / generators / silhouettes, panels + hatching,
   lines, blades. Output goes straight into the Wire's buckets. */
(function () {
  'use strict';
  const { V, X } = M3;
