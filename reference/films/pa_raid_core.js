...
(function () {
  ...
  PA.tag = o => req.push(o);
  PA.flushTags = function (octx, uiEl, menuBox, blocks) {
    req.sort((a, b) => (b.pri || 0) - (a.pri || 0) || a.y - b.y);
    // blocks: screen boxes other overlays already hold (the class bars); tags step round them
    const placed = blocks ? blocks.slice() : [];
    for (const q of req) {
      ...
      for (let g = 0; g < 10; g++) {
        const hit = placed.find(p => x < p.x + p.w + 6 && p.x < x + w + 6 && y < p.y + (p.h || 20) + 4 && p.y < y + 24);
        if (!hit) break; y = q.down ? hit.y + (hit.h || 20) + 5 : hit.y - 25;
      }
