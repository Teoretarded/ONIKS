/* SURVEY · BATTLE (Point Cloud B, combat edition) · the world, precomputed once at load.
   ...
   model sampled into dots. The film script (pb_battle_film.js) only looks things up. */
window.PBS = (function () {
  'use strict';
  ...
  // TRK 02 keeps station on the leader's starboard quarter: [starboard, forward] metres in its frame
  const SHIP2 = { off: [520, -540], hdg: SHIP.hdg, v: SHIP.v, d: SHIP.d };
  {
    const rt = [Math.cos(SHIP.hdg), -Math.sin(SHIP.hdg)], o = SHIP2.off;
    SHIP2.dx = rt[0] * o[0] + SHIP.d[0] * o[1]; SHIP2.dz = rt[1] * o[0] + SHIP.d[1] * o[1];
    SHIP2.at = s => { const c = SHIP.at(s); c[0] += SHIP2.dx; c[1] += SHIP2.dz; return c; };
  }
  ...
  Object.assign(S, { SHIP, SHIP2, SQ, WIND });
  ...
  const ANCH = [
    ...
    // the engagement: real time, easing into x0.5 through the fight, back up into the squall.
    // Each segment's sim span is chosen so its plateau is exactly the rate named (see the ramp formula below)
    { T: 88, s: EV.orbitIn, r: 8 },
    { T: 103, s: EV.orbitOut - 12.295, r: 1 },
    { T: 107, s: EV.orbitOut - 8.75, r: .5 },
    { T: 112.5, s: EV.orbitOut - 6, r: .5 },
    { T: 114.5, s: EV.orbitOut - 4, r: 1.5 },
